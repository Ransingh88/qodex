...
const updateUserProfile = asyncHandler(async (req, res) => {
  ...
  const updatedUser = await User.findByIdAndUpdate(req.user?._id, { $set: updatePayload }, { new: true }).select(
    "-password -refreshToken"
  );

  res.status(200).json(new APIResponse(200, "user data updated successfully", updatedUser));
});
